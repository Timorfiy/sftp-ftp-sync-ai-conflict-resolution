...
```jsonc
{
  ...
  "conflictCheck": false,                   // Block unsafe overwrites using the last observed remote metadata. Default: false.
...
The `remoteExplorer`, `watcher`, and `syncOption` objects are shallow-replaced,
not deep-merged. The `backup` object is merged with its defaults so omitted
children retain their configured defaults. Profiles use the same replacement
rules; only `ignore` is concatenated with the top-level value.
...
When `true`, an upload of an existing remote file is allowed only when its exact
modification time and byte size still match the last remote version observed by
this workspace. The baseline is refreshed after successful uploads and
